...
OPENCLI_FALLBACK_ENABLED = os.environ.get("DAILY_INTEL_OPENCLI_FALLBACK", "1") != "0"
OPENCLI_RETRIES = int(os.environ.get("DAILY_INTEL_OPENCLI_RETRIES", "2"))
OPENCLI_TIMEOUT_SECONDS = int(os.environ.get("DAILY_INTEL_OPENCLI_TIMEOUT_SECONDS", "90"))
OPENCLI_PROFILE = os.environ.get("DAILY_INTEL_OPENCLI_PROFILE", "").strip()
README_CANDIDATES = [
    "README.md",
    "README.MD",
    "readme.md",
...
def opencli_read_markdown(url):
    if not OPENCLI_FALLBACK_ENABLED:
        return {"ok": False, "error": "opencli fallback disabled by DAILY_INTEL_OPENCLI_FALLBACK=0"}

    last_error = ""
    for attempt in range(1, OPENCLI_RETRIES + 1):
        cmd = ["opencli"]
        if OPENCLI_PROFILE:
            cmd.extend(["--profile", OPENCLI_PROFILE])
        cmd.extend([
            "web",
            "read",
            "--url",
        ...
        try:
            ...
            return {"ok": False, "error": "opencli executable not found"}
        except subprocess.TimeoutExpired:
            last_error = f"opencli web read timed out after {OPENCLI_TIMEOUT_SECONDS}s"
            continue

        stdout = proc.stdout.decode("utf-8", errors="replace")
        stderr = proc.stderr.decode("utf-8", errors="replace").strip()
        body = clean_opencli_output(stdout)
        ...
        if proc.returncode == 0:
            last_error = "opencli web read returned short or unusable content"
        else:
            last_error = stderr or clean_opencli_output(stdout) or f"opencli exit {proc.returncode}"

        if attempt < OPENCLI_RETRIES:
            time.sleep(2)

    return {"ok": False, "error": last_error or "opencli web read failed"}
...
def fetch_readable_page(url, output_dir, stem, fetched=None):
    ...
    if fallback.get("ok"):
        ...
        opencli_path = write_archive_text(output_dir / f"{stem}.opencli.md", body)
        result.update(
            {
                "fulltext_status": "ok",
                "fulltext_method": "opencli-read",
                "fulltext_path": opencli_path,
                "fulltext_chars": len(readable),
                "fulltext_excerpt": text_excerpt(readable),
                "fallback_reason": curl_reason,
                "opencli_attempts": fallback.get("attempts"),
            }
        )
    ...
    result.update(
        {
            ...
            "fulltext_method": "curl",
            "fulltext_error": f"{curl_reason}; opencli fallback failed: {fallback.get('error') or 'unknown error'}",
        }
    )
    return result
                ...
                results.append(
                    {
                        ...
                        "url": source["url"],
                        "status": "limited",
                        "reason": "GitHub Trending curl fetch failed; opencli-readable snapshot archived, but repo-card parser requires raw Trending HTML.",
                        "items": [],
                        **fallback,
                    }
                )
    ...
    for source in official_sources:
        ...
        if html_looks_limited(fetched["body"]):
            ...
            if fallback.get("fulltext_status") == "ok":
                ...
                record["reason"] = "HTML page returned limited/challenge content; opencli fallback did not produce readable text."
            ...
            if fallback.get("fulltext_status") == "ok":
                ...
                record.update(fallback)
                record["status"] = "limited"
                record["reason"] = "HTML page did not return a usable title/content snapshot; opencli fallback did not produce readable text."
            ...
            if fallback.get("fulltext_status") == "ok":
                ...
                record.update(fallback)
                record["status"] = "limited"
                record["reason"] = "Redirected to platform.claude.com and returned region-unavailable HTML; opencli fallback did not produce readable text."
