...
网页或公开文件的 `curl` 抓取失败、Cloudflare/JS challenge、正文太短或不可读时，collector 会尝试 `opencli web read`。如果成功，本地归档使用 `.opencli.md`，证据方法写作 `opencli-read`；如果仍失败，日报必须保留 `needs-fulltext` / `limited` 边界。
...
中文译读阶段已退役。每日自动化没有中文译读 runner，不设置翻译相关环境变量，也不再维护 `translations/YYYY-MM-DD/` 下的 manifest、index、shard 或 `.zh.md` 派生产物。

证据真相源仍是 [`../raw/`](../raw/) 与 [`../trend/raw/`](../trend/raw/) 中的本地归档，以及日报、candidate audit、trend report 和专题报告中的中文分析正文。完成条件不包含 translation check；如需阅读原文，直接使用 raw 归档材料。
